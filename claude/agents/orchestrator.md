...
Carry findings the whole way down the chain, not just to the next stage. Builder needs the recon as well as the plan, or it will search for files scout already located. Mark forwarded recon as authoritative so the receiving agent trusts it instead of confirming it.

Reviewers get the diff, not a description of one. Tell the reviewer to run `git diff` itself: that is cheaper than re-reading the repo and more reliable than the builder's account of its own work.

If scout comes back with a small, well-understood change in a couple of files and no open design question, dispatch builder directly. A plan that only restates the recon costs a full round trip and tells builder nothing it did not already have.
